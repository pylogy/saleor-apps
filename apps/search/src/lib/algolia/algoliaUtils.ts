...
const mapSelectedAttributesToRecord = (attr: ProductAttributesDataFragment) => {
  ...
  let value: string | boolean | Array<string | null | undefined>;
  ...
  if (isAttributeValueBooleanType(filteredValues)) {
    ...
    // Handle tags attribute value as list
    if (attr.attribute.name.toLowerCase() == "tags") {
      value = filteredValues.map((v) => v.name) || [];
    } else {
  ...
  } as Record<string, string | boolean | Array<string | null | undefined>>;
};
